 ...
 * Date: 2017/7/27
 * Time: 16:01
 * Desc: copy from {@link AnimatedDrawableBackendImpl}, 但是修改了{@link #renderImageSupportsScaling(Canvas, AnimatedImageFrame)}
 *       和{@link #renderImageDoesNotSupportScaling(Canvas, AnimatedImageFrame)}方法，让头像直接呈现圆形
 */

public class AnimatedDrawableBackendImplSupport extends AnimatedDrawableBackendImpl {
    ...
    public AnimatedDrawableBackendImplSupport(AnimatedDrawableUtil animatedDrawableUtil,
                                              AnimatedImageResult animatedImageResult, Rect bounds) {
        super(animatedDrawableUtil, animatedImageResult, bounds);
    ...
    private void renderImageSupportsScaling(Canvas canvas, AnimatedImageFrame frame) {
        ...
        synchronized (this) {
            ...
            BitmapShader shader =
                    new BitmapShader(mTempBitmap, BitmapShader.TileMode.CLAMP, BitmapShader.TileMode.CLAMP);
            paint.setShader(shader);
            paint.setAntiAlias(true);
            float renderedBoundsWidth = mRenderedBounds.width();
            canvas.drawCircle(renderedBoundsWidth / 2f, renderedBoundsWidth / 2f, renderedBoundsWidth / 2f, paint);
        }
    }
